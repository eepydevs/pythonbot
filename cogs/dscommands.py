...
class dscommands(commands.Cog):
    ...
    @commands.command(
        help=
        "Second command! This time, with a text paramater! Still do not know what it does though.",
        description="Usage: pb!texttest (text)")
    async def texttest(self, ctx, *, text=None):
        if text == None:
            await ctx.send("No text found! Please add text for it to work.")
        else:
            await ctx.send(
                f"Testing command! My second ever command.\nAlso {textSM(text)}!"
            )

    @commands.command(
        help="I do not know! Now with a number and text parameter.",
        description="Usage: pb!nwpttest")
    async def nwpttest(self, ctx, number, *, text):
        await ctx.send(
            f"Testing command! My third ever command. {number}, {text}. What do they have in common? They are all typed by {ctx.author.name}!"
        )

    @commands.command(
        help="Calculator, but wrong. Addition only supported right now.",
        description="Usage: pb!calcwrong number1 number2")
    async def calcwrong(self, ctx, number1, number2):
        try:
            await ctx.send(
                f"{number1} + {number2} = {random.randint(-int(number1), int(number2))}"
            )
        except ValueError:
    ...
    @commands.command(
        help=
        "Makes your sentence look like it came from someone with dyslexia.",
        description="Usage: pb!dys text")
    async def dys(self, ctx, *, text):
        modtext = " ".join(str().join(random.sample(i, len(i)))
                           for i in text.split())
